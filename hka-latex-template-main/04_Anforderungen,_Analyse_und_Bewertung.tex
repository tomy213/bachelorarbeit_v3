...
\subsection*{Radar-KPIs \& Abnahmekriterien (ODD-segmentiert)}
\begin{itemize}
  \item \textbf{Detektion/Klassifikation:} $P_d(\tau)$, $P_{fa}(\tau)$, ROC/PR \emph{je} Range-/SNR-Band; klassenweise F1/MCC.
  \item \textbf{Kalibrierung/Unsicherheit:} ECE, Brier, NLL \emph{je Segment}; Bootstrap-Konfidenzbänder.
  \item \textbf{OOD/Unknown:} OOD-TPR@1\%FPR, Abstain-Rate, Auswirkung auf $P_d/P_{fa}$.
  \item \textbf{Laufzeit/Verfügbarkeit:} P95/P99-Latenz, Jitter, Verfügbarkeit pro Pfad.
  \item \textbf{Ressourcen:} RAM/Flash/Compute-Budget (Inline) auf Zielhardware.
\end{itemize}
\noindent\emph{Abnahmeprinzip:} Bewertung \emph{pro ODD-Segment}; Gesamtmittelwerte nur ergänzend.
...
\subsection*{Modellauswahl-Logik (0–3 Eignung, indikativ)}
\begin{itemize}
  \item \textbf{CNN (leicht, RD/RDA):} Leistung~3, Kalibrierung~2, OOD~2, Latenz~2–3, Repro/Audit~2.
  \item \textbf{Deep Ensembles \textit{oder} MC-Dropout:} Leistung~3, Kalibrierung~3, OOD~3, Latenz~1–2, Repro/Audit~2.
  \item \textbf{GBDT (Tracks/Tabular):} Leistung~2, Kalibrierung~2, OOD~1–2, Latenz~3, Repro/Audit~3.
  \item \textbf{Autoencoder/One-Class (Anomalie):} Neuheit~2, Kalibrierung~1, OOD~2, Latenz~2–3, Repro/Audit~2.
  \item \textbf{Temporal (TCN/Transformer):} Sequenznutzen~2–3, Latenz~1–2, Repro~2.
\end{itemize}
\noindent\emph{Regel:} Nur Modelle wählen, die \emph{pro Segment} KPI \& Latenz/Ressourcen erfüllen \emph{und} eine nachvollziehbare Kalibrierungs-/OOD-Strategie haben.
...
\subsection*{Empfehlungs-Schnittmenge (Go-Forward-Set)}
\begin{itemize}
  \item \textbf{Kernklassifikator:} leichtes CNN auf RD/RDA + \textit{Deep Ensemble} (N=3–5) \textit{oder} MC-Dropout.
  \item \textbf{Kalibrierung:} Temperature Scaling \emph{je ODD-Segment}; ECE/Brier/NLL mit CI.
  \item \textbf{OOD/Abstain:} Energie-Score \textit{oder} Mahalanobis im Feature-Space; segment-spezifische Schwellen; Hysterese \& Operator-Hinweis.
  \item \textbf{Anomalie (optional, Frontloading/Monitoring):} Autoencoder/One-Class für Drift-/Neuheit, nicht als Primärpfad.
  \item \textbf{Tabulare Baseline (Frontloading):} GBDT für Analysen, Plausibilisierung, Schwellenableitung.
\end{itemize}
...
\subsection*{Abnahmeplan}
\begin{enumerate}
  \item \textbf{Daten/ODD:} Szenario-Steckbriefe, Mindest-$n$ je Segment, getrennte Splits pro Segment.
  \item \textbf{Leistung:} Pd/Pfa, ROC/PR je Segment; Bootstrap-CI; Worst-Case-Reports.
  \item \textbf{Kalibrierung:} Reliability-Plots, ECE/Brier/NLL je Segment; dokumentierte Freigabeschwellen.
  \item \textbf{OOD/Abstain:} OOD-TPR@1\%FPR; Einfluss von Abstain auf Pd/Pfa; Operator-Playbook.
  \item \textbf{Robustheit:} Stress-/Shift-Tests (Clutter/RFI/Wetter/Aspekt); Degradationskurven; Akzeptanzkriterien.
  \item \textbf{Laufzeit:} P95/P99-Latenz/Jitter auf Zielhardware; CI/CD-Regression-Gates.
  \item \textbf{Repro/Toolchain:} Seeds, Lockfiles, Artefakt-IDs, Build-Manifeste; ggf. TQL-Entscheidung (DO-330).
\end{enumerate}
