...
\section{Modell-Steckbriefe für Onboard-Radar (Wetterklassifikation)}
...
\begin{itemize}
  ...
  \item \textbf{Input/Features:} 2D/2.5D Radar-Schnitte, Textur/Gradienten, Sequenzen, Radarrohdaten möglich.
  \item \textbf{Stärken:} Hohe Genauigkeit; lernt komplexe Muster; edge-tauglich mit leichten Backbones \& Quantisierung.
...
\subsection*{Probabilistische \& OOD-Modelle}
\begin{itemize}
  \item \textbf{Zweck:} Verlässliche Unsicherheit, Out-of-Distribution \& Anomalie-Erkennung.
  \item \textbf{Typische Variante:} Deep Ensembles
  \item \textbf{Input/Features:} Ausgaben von CNN/GBDT + Qualitätsflags (z.B. SNR, RFI).
  \item \textbf{Stärken:} Liefert \emph{Confidence} statt nur harte Klassen; erkennt ungewöhnliche Situationen.
  \item \textbf{Schwächen:} Höherer Rechenbedarf bzw.\ tuning-sensitiv (OOD-Schwellen); komplexere Evaluation.
  \item \textbf{Latenz/Compute:} Ensembles 2--5$\times$ Einzelmodell; nicht Echtzeit fähig
\end{itemize}

\subsection*{Clustering}
\begin{itemize}
  \item \textbf{Zweck:} Objektbildung, Vor-/Nachverarbeitung, Track-Seeding, Daten-Kuration (nicht primär finale Feinklassen).
...
\textbf{Ergänzend} liefert Unsicherheit/OOD, keine Semantik allein &
...
\textbf{Steigert Verlässlichkeit} , keine eigene F1 &
\textbf{Parameterabhängig} keine Klassenmetriken \\

\textbf{Latenz} &
\textbf{Erfüllbar}&
\textbf{Sehr gut} ($<\,$ms/Objekt) &
\textbf{2–5$\times$} Basislatenz (Ensembles) &
\textbf{Gut} \\
...
\textbf{Nein} ergänzendes Modul &
\textbf{Nein} Vor-/Nachverarbeitung \\
\end{longtable}

Als alleinstehendes Modell empfehlen sich Baummodelle (so fern man gute Features hat). 
...
Das hat den Vorteil, dass es nicht bereits an der Hardwarezulassung scheitert um dieses Modell theoretisch verwenden zu können.

\section{Einbindungsmöglichkeiten}
Als assistierendes System neben dem klassischen zur zusätzlichen Informationsgewinnung.\\
Inline anstatt klassischer Schwellwertentscheider\\
(inline bayes klassifikator? oder wie war das nochmal)\\
\\
mögliche szenarien beim assistieren oder im vergleich zum klassischen:\\
- stimmen überein   gut\\
- sind nah bei einander   kann leicht problematisch werden\\
- eines fällt aus     kommt auf situation an\\
- sie sind gegeteilig     interessant: was tun?\\
